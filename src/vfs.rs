...
pub trait NFSFileSystem: Send + Sync {
    ...
    fn capabilities(&self, auth: &auth_unix) -> VFSCapabilities;

    /// Returns the handle of the root directory "/"
    fn root_dir(&self, auth: &auth_unix) -> Self::FileHandle;

    /// Look up the handle of a path in a directory
    async fn lookup(&self, auth: &auth_unix, dir_handle: &Self::FileHandle, filename: &filename3)
                    -> Result<Self::FileHandle, nfsstat3>;

    /// Returns the attributes of a file
    async fn getattr(&self, auth: &auth_unix, handle: &Self::FileHandle) -> Result<fattr3, nfsstat3>;

    /// Sets the attributes of a file
    /// Returns Err(nfsstat3::NFS3ERR_ROFS) if readonly
    async fn setattr(&self, auth: &auth_unix, handle: &Self::FileHandle, setattr: sattr3) -> Result<fattr3, nfsstat3>;

    /// Reads the contents of a file returning (bytes, EOF)
    async fn read(&self, auth: &auth_unix, handle: &Self::FileHandle, offset: u64, count: u32)
                  -> Result<(Vec<u8>, bool), nfsstat3>;

    /// Writes the contents of a file
    /// Returns Err(nfsstat3::NFS3ERR_ROFS) if readonly
    async fn write(&self, auth: &auth_unix, handle: &Self::FileHandle, offset: u64, data: &[u8])
    ...
    /// Removes a file
    /// Returns Err(nfsstat3::NFS3ERR_ROFS) if readonly
    async fn remove(&self, auth: &auth_unix, dir_handle: &Self::FileHandle, filename: &filename3)
                    -> Result<(), nfsstat3>;
    ...
    async fn readdir_simple(
        ...
        let result = self.readdir(auth, dir_handle, 0, count).await?;
        Ok(result.to_simple())
    }
    ...
    /// Reads a symlink
    async fn readlink(&self, auth: &auth_unix, handle: &Self::FileHandle) -> Result<nfspath3, nfsstat3>;
    ...
    async fn write_with_stability(
        ...
        Ok((self.write(auth, handle, offset, data).await?, stable_how::FILE_SYNC))
    }
    ...
    async fn fsinfo(
        ...
        let dir_attr: nfs::post_op_attr = match self.getattr(&auth, root_handle).await {
    ...
    async fn path_to_handle(&self, auth: &auth_unix, path: &[u8]) -> Result<Self::FileHandle, nfsstat3> {
        let splits = path.split(|&r| r == b'/');
        let mut handle = self.root_dir(auth);
        for component in splits {
            if component.is_empty() {
                continue;
            }
            handle = self.lookup(&auth, &handle, &component.into()).await?;
        }
        Ok(handle)
    }

    /// Returns a unique server ID
    fn serverid(&self, auth: &auth_unix) -> cookieverf3 {
