...
use crate::rpc::*;
use crate::vfs::{FromNfsFh, NFSFileSystem, VFSCapabilities};
use crate::xdr::*;
    ...
    VFS: NFSFileSystem,
{
    ...
    match prog {
        ...
        NFSProgram::NFSPROC3_RMDIR => nfsproc3_rmdir(xid, input, output, context).await?,
 ...
 */
pub async fn nfsproc3_getattr<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let file_handle = VFS::FileHandle::from_nfs_fh(&handle);
    if let Err(stat) = file_handle {
...
*
*/
pub async fn nfsproc3_lookup<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let dir_handle = VFS::FileHandle::from_nfs_fh(&dirops.dir);
    // fail if unable to convert file handle
    if let Err(stat) = &dir_handle {
        make_success_reply(xid).serialize(output)?;
        stat.serialize(output)?;
    ...
    match context.vfs.lookup(&dir_handle, &dirops.name).await {
        Ok(file_handle) => {
            ...
            make_success_reply(xid).serialize(output)?;
            nfs::nfsstat3::NFS3_OK.serialize(output)?;
            let handle_nfs: nfs::nfs_fh3 = file_handle.clone().into();
            handle_nfs.serialize(output)?;
 ...
 */
pub async fn nfsproc3_read<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let handle = VFS::FileHandle::from_nfs_fh(&args.file);
    if let Err(stat) = handle {
        make_success_reply(xid).serialize(output)?;
        stat.serialize(output)?;
        nfs::post_op_attr::Void.serialize(output)?;
        return Ok(());
...
pub async fn nfsproc3_fsinfo<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let id = VFS::FileHandle::from_nfs_fh(&handle);
    // fail if unable to convert file handle
    if let Err(stat) = id {
        make_success_reply(xid).serialize(output)?;
        stat.serialize(output)?;
...
pub async fn nfsproc3_access<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let granted = if let nfs::post_op_attr::attributes(ref a) = obj_attr {
        let is_dir = matches!(a.ftype, nfs::ftype3::NF3DIR);
        let mut g = ACCESS3_READ | ACCESS3_MODIFY | ACCESS3_EXTEND;
        if is_dir {
            g |= ACCESS3_LOOKUP | ACCESS3_DELETE;
        } else if a.mode & 0o111 != 0 {
            g |= ACCESS3_EXECUTE;
        }
        g
    } else {
    ...
    let access = if matches!(context.vfs.capabilities(), VFSCapabilities::ReadWrite) {
        access & granted
    } else {
...
*/
pub async fn nfsproc3_pathconf<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
...
pub async fn nfsproc3_fsstat<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
...
*/
pub async fn nfsproc3_readdirplus<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let dir_handle = VFS::FileHandle::from_nfs_fh(&args.dir);
    // fail if unable to convert file handle
    if let Err(stat) = dir_handle {
        make_success_reply(xid).serialize(output)?;
        stat.serialize(output)?;
        ...
        .await
    {
        Ok(result) => {
            ...
            for entry in result.entries {
                ...
                // let handle = nfs::post_op_fh3::handle(entry.into());
                let entry = entryplus3 {
                    fileid: obj_attr.fileid,
                    name: entry.name,
                    cookie: entry.cookie,
                    name_attributes: nfs::post_op_attr::attributes(obj_attr),
                    name_handle: nfs::post_op_fh3::handle(entry.handle.into()),
                };
            ...
            debug!(
                ...
                &dir_handle, has_version, args.cookie, ctr, all_entries_written
            );
        }
        Err(stat) => {
...
pub async fn nfsproc3_readdir<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let dirid = VFS::FileHandle::from_nfs_fh(&args.dir);
    // fail if unable to convert file handle
 ...
 */
pub async fn nfsproc3_write<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let id = VFS::FileHandle::from_nfs_fh(&args.file);
    if let Err(stat) = id {
        make_success_reply(xid).serialize(output)?;
        stat.serialize(output)?;
        nfs::wcc_data::default().serialize(output)?;
    ...
    match context
        .vfs
        .write_with_stability(&id, args.offset, &args.data, stable_how)
        .await
    {
        Ok((fattr, committed)) => {
            debug!(
...
pub async fn nfsproc3_commit<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
...
pub async fn nfsproc3_create<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    // new file in
    let dirid = VFS::FileHandle::from_nfs_fh(&dirops.dir);
    if let Err(stat) = dirid {
    ...
    let fid: Result<VFS::FileHandle, nfs::nfsstat3>;
    ...
    if matches!(createhow, createmode3::EXCLUSIVE) {
        ...
        (fid, postopattr) = res.map_or_else(
            |err| (Err(err), nfs::post_op_attr::Void),
            |(fid, fattr)| (Ok(fid), nfs::post_op_attr::attributes(fattr)),
        )
    }
    ...
    match fid {
        Ok(fid) => {
            ...
            nfs::post_op_fh3::handle(fid.clone().into()).serialize(output)?;
...
pub async fn nfsproc3_setattr<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let id = VFS::FileHandle::from_nfs_fh(&args.object);
    // fail if unable to convert file handle
    if let Err(stat) = id {
        make_success_reply(xid).serialize(output)?;
        stat.serialize(output)?;
...
async fn nfsproc3_remove_impl<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    // RFC 1813: REMOVE must return NFS3ERR_ISDIR for directories;
    //           RMDIR must return NFS3ERR_NOTDIR for non-directories.
    match context.vfs.lookup(&dirid, &dirops.name).await {
        Ok(target_fh) => {
            match context.vfs.getattr(&target_fh).await {
                Ok(target_attr) => {
                    let is_dir = matches!(target_attr.ftype, nfs::ftype3::NF3DIR);
...
pub async fn nfsproc3_remove<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
...
where
    VFS: NFSFileSystem,
{
    nfsproc3_remove_impl(xid, input, output, context, false).await
}

pub async fn nfsproc3_rmdir<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
...
where
    VFS: NFSFileSystem,
{
    nfsproc3_remove_impl(xid, input, output, context, true).await
}
...
pub async fn nfsproc3_rename<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    let from_dirid = VFS::FileHandle::from_nfs_fh(&fromdirops.dir);
    ...
    let to_dirid = VFS::FileHandle::from_nfs_fh(&todirops.dir);
...
pub async fn nfsproc3_mkdir<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
    ...
    VFS: NFSFileSystem,
{
    ...
    // find the directory we are supposed to create the
    // new file in
    let dirid = VFS::FileHandle::from_nfs_fh(&args.dirops.dir);
    if let Err(stat) = dirid {
        // directory does not exist
        make_success_reply(xid).serialize(output)?;
        stat.serialize(output)?;
    ...
    match res {
        Ok((fid, fattr)) => {
            ...
            nfs::nfsstat3::NFS3_OK.serialize(output)?;
            // serialize CREATE3resok
            nfs::post_op_fh3::handle(fid.into()).serialize(output)?;
...
pub async fn nfsproc3_symlink<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
...
*/
pub async fn nfsproc3_readlink<VFS>(
    xid: u32,
    input: &mut impl Read,
    output: &mut impl Write,
    context: &RPCContext<VFS>,
